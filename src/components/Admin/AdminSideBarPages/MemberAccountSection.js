...
const MemberAccountSection = () => {
  ...
  const callApiToGetMemberToalBalance = () => {
    ...
    axios
      .post(
        `${apiurl}` + "/admin/fetch-particular-member-details-using-memberid",
        data,
        config
      )
      .then((res) => {
  ...
  const callApiToGetRequestHistory = () => {
    ...
    axios
      .post(
        `${apiurl}` + "/admin/fetch-refferal-payout-withdrawal-request",
        data,
        config
      )
      .then((res) => {
  ...
  const handleApprove = (id) => {
    ...
    Modal.confirm({
      ...
      onOk() {
        axios
          .post(
            `${apiurl}` + "/admin/approve-member-refferal-payout",
            requestData,
            config
          )
          .then((res) => {
  ...
  const callApiToGetApprovedHistory = () => {
    ...
    axios
      .post(
        `${apiurl}` + "/admin/fetch-member-refferal-payout-approve-withdrawal",
        data,
        config
      )
      .then((res) => {
  ...
  const callApiToGetMemberBankDetails = () => {
    ...
    axios
      .post(`${apiurl}` + "/admin/get-member-bak-and-upi-details", data, config)
      .then((res) => {
      ...
      <div className="state-account-container">
        <div className="state-account-header">
          <div className="d-flex ">
            <BiArrowBack onClick={gotoHome} style={{ cursor: "pointer",color:'wheat' }} />Member Account
          </div>
      ...
      <Tabs defaultActiveKey="1">
        ...
        <TabPane tab="Approved History" key="2">
          <Table
            ...
            dataSource={approvedMemberDetails}
            scroll={{ x: 800, y: 350 }}
          />
        </TabPane>
        <TabPane tab="Bank Details" key="3">
          ...
          {selectedValue === 1 ? (
            <Table
              ...
              dataSource={memberBankDetails}
              scroll={{ x: 800, y: 350 }}
            />
          ) : (
            <Table
              columns={columnMemberUpi}
              dataSource={memberUpiDetails}
              scroll={{ x: 800, y: 350 }}
            />
          )}
        </TabPane>
